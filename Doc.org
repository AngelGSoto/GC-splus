...
# Guía práctica: Uso de filtros personalizados S-PLUS en CIGALE (v2025.x)

## 1. Objetivo del flujo

Permitir el ajuste de SEDs en cúmulos globulares usando fotometría S-PLUS y filtros personalizados en CIGALE, con **configuración moderna y robusta**.

---

## 2. Archivos y estructura necesaria

En tu directorio de trabajo debe haber:

- **Datos de entrada**  
  Archivo tipo CSV o txt con columnas (ejemplo):  
  `id, redshift, F0378, F0378_err, F0395, F0395_err, ...`  
  (ejemplo de nombre: `gc_splus_cigale_custom.txt`)

- **Archivos de transmisión de filtros:**  
  `F0378.dat`, `F0395.dat`, ..., `F0861.dat`  
  (formato y generación explicados abajo)

---

## 3. Formato exacto del archivo de filtro para CIGALE

**Cada archivo debe lucir así:**
...
... (más datos)
```
- La línea `energy` (o `photon` si corresponde) **NUNCA lleva `#`**.
- Comentarios con `#` permitidos antes o después.
- Luego, sólo datos numéricos: longitud de onda [Å] y transmisión.

> ⚠️ **NO agregues ninguna línea tipo "Wavelength Transmission", solo los datos numéricos.**

---

## 4. Script recomendado para crear/convertir los filtros

Aquí tienes el script Python para transformar cualquiera de tus archivos originales en archivos válidos para CIGALE:
...
def crear_filtros_formato_exacto():
    ...
    for filtro, descripcion in filtros_info.items():
        ...
        archivo_salida = f"{filtro}_cigale.dat"
        if os.path.exists(archivo_entrada):
            with open(archivo_entrada, 'r') as f:
                datos_originales = [l for l in f if l.strip() and not l.lstrip().startswith("#")]
            with open(archivo_salida, 'w') as f:
                f.write(f"# {filtro}\n")
                f.write("energy\n")
                f.write(f"# {descripcion}\n")
                for linea in datos_originales:
                    f.write(linea)
            print(f"✅ Generado {archivo_salida}")
        else:
            print(f"❌ Falta {archivo_entrada}")
...
## 5. Importar los filtros personalizados a la base de datos de CIGALE

Después de tener los archivos correctos (ejemplo, `F0378_cigale.dat`):

```bash
pcigale-filters add F0378_cigale.dat F0395_cigale.dat F0410_cigale.dat F0430_cigale.dat F0515_cigale.dat F0660_cigale.dat F0861_cigale.dat
```

Puedes verificar que fueron añadidos exitosamente con:

```bash
pcigale-filters list | grep F0
```
Debes ver tus nombres, descripciones y longitudes de onda.

---

## 6. Generar y editar el archivo de configuración de CIGALE

1. **Genera prototipo de configuración solo la primera vez:**
    ```bash
    pcigale genconf
    ```
   CIGALE creará/actualizará `pcigale.ini`.

2. **Edítalo a mano** y asegúrate de incluir SOLO tus bandas puras así (el nombre coincide con el listado anterior):

    ```ini
    bands = F0378, F0395, F0410, F0430, F0515, F0660, F0861
    ```

    NO pongas `_err`, SOLO el nombre puro (CIGALE usará el error si existe la columna *_err*).

3. Asegúrate de tener los módulos (ejemplo):
    ...
    Ajusta los bloques de los módulos y los parámetros según tus necesidades científicas, inspirándote en la plantilla y tus necesidades (ver ejemplos previos).

---

## 7. Flujo robusto de trabajo y comandos finales

**NUNCA vuelvas a usar `pcigale genconf` después de tu edición manual del `.ini`, pues borraría tus bandas.**
...
# 2. (Si actualizaste un filtro)
pcigale-filters add F0*.dat  # o tus *_cigale.dat

# 3. Edita y verifica el archivo .ini con las bandas correctas
...
# 6. (Opcional) Visualiza plots/fits:
pcigale plot
```
...
- **Tus bandas deben aparecer en la salida de `pcigale-filters list`.**
- Al hacer `pcigale check`, deben salir como “Bands fitted” y SIN warnings de “not taken into account in the fits.”
- **Nunca dejes líneas tipo “Wavelength Transmission” antes de los datos en tus .dat.**

---

## 9. Referencias útiles

- [CIGALE documentación oficial](https://cigale.lam.fr/documentation/)
- [Tutorial: Agregar filtros personalizados](https://cigale.lam.fr/documentation/tutorials/adding-filters/)

---
**Cualquier duda, verifica la documentación o ejecuta cada comando con el flag `--help`.**
