...
plt.ylabel(r'rSDSS - H$\alpha$', fontsize= 16)
...
ax1.legend(scatterpoints=1, ncol=2, fontsize=5.8, loc='upper left', **lgd_kws)
ax1.grid()
#lgd = ax1.legend(loc='center right', bbox_to_anchor=(1.27, 0.5), fontsize=7.5, **lgd_kws)
#ax1.grid(which='minor', lw=0.5)
#sns.despine(bottom=True)
plt.tight_layout()
plt.savefig('luis-JPAS-Viironen.pdf')#,  bbox_extra_artists=(lgd,), bbox_inches='tight')
plt.clf()
...
#ax2.set_xlim(xmin=-1.7,xmax=2.0)
ax2.set_xlim(xmin=-5.2,xmax=2.0)
ax2.set_ylim(ymin=-4.1,ymax=2.0)
plt.tick_params(axis='x', labelsize=15) 
plt.tick_params(axis='y', labelsize=15)
plt.xlabel(r'H$\alpha$ - rSDSS', fontsize= 16)
plt.ylabel(r'H$\alpha$ - [O III]', fontsize= 16)
...
ax2.legend(scatterpoints=1, ncol=2, fontsize=5.8, loc='upper left', **lgd_kws)
###ax2.grid()
