'''
Principal component analysis and linear discriminant analysis (S-PLUS internal DR2)---Predicting
'''
...
lda.fit(XX_pca_new, y_new)

XX_lda = lda.transform(XX_pca_new)
...
select(0.0, "PN-pcld-{}.tab".format(File_name.split('.tab')[0]))
select(1.0, "SySt-pcld-{}.tab".format(File_name.split('.tab')[0]))
sys.exit()   

m0 = y_prob[:,0] >= 0.8
m1 = y_prob[:,1] >= 0.8
tab[m0].write("PN-pcld-{}.tab".format(File_name.split('2_')[-1].split('.tab')[0]), format='ascii.tab', overwrite=True)
tab[m1].write("SySt-pcld-{}.tab".format(File_name.split('2_')[-1].split('.tab')[0]), format='ascii.tab', overwrite=True)
