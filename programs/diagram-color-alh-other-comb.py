...
for line in f:
    line = line.strip()
    columns = line.split()
    mag1 = float(columns[25])
    mag2 = float(columns[15])
    mag3 = float(columns[23])
    d_mag1 = mag1 - mag2
    d_mag2 = mag1 - mag3
...
ax1.set_xlim(xmin=-4.8,xmax=2.0)
ax1.set_ylim(ymin=-4.3,ymax=2.0)
plt.xlabel('F644 - F613', size = 12)
plt.ylabel('F644 - F489', size = 12)
...
ax1.scatter(d_768_sys, d_644_sys,  c= "red", alpha=0.8, marker='s', label='Symbiotics in NGC')
...
ax1.legend(scatterpoints=1, ncol=2, fontsize=5.9, loc='lower left', **lgd_kws)
#ax1.grid()
ax1.grid(which='minor', lw=0.5)
#sns.despine(bottom=True)
plt.tight_layout()
plt.savefig('diagram-alhambra-F644.pdf')
