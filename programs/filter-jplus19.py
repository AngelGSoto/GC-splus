...
# data = ascii.read('JPLUS_gSDSS_FullEfficiency.tab')
# print(data['col1'])
...
for f, color, filter_ in zip(files, colors, filters):
    x, y = load_file(f)
    plt.fill(x,y, color=color, label=filter_, zorder=3.0)

for f, color, filter_ in zip(files1, colors1, filters1):
    x, y = load_file(f)
    plt.fill(x,y,  color=color, linewidth=2.8, alpha=0.3, label=filter_)
...
plt.plot(x, y, linewidth=1.3, color="k", zorder=5.0)
#plt.ylim(0.0, 1.0)    
