'''
Make colour-colour diagram to a particular filters
'''
...
#groups = [ "'QSOs (0.01<z<1.0)", "QSOs (1.01<z<2.0)", "'QSOs (2.01<z<3.0)", "'QSOs (3.01<z<4.0)", "'QSOs (4.01<z<5.0)", ]
#colors = [ "light brown", "light orange", "pale yellow", "dark pink", "cerulean", ]
...
for file_name in file_list:
    with open(file_name) as f:
        ...
        x22, y22 = filter_mag("DPNe")
        x23, y23 = filter_mag("QSOs-hz")
        x24, y24 = filter_mag("QSOs-010")
        x25, y25 = filter_mag("QSOs-101")
        x26, y26 = filter_mag("QSOs-201")
        x27, y27 = filter_mag("QSOs-301")
        x28, y28 = filter_mag("QSOs-401")
        x29, y29 = filter_mag("-SFGs")
        x30, y30 = filter_mag("-sys")
        x31, y31 = filter_mag("-sys-IPHAS") 
        x32, y32 = filter_mag("-ExtHII")
        x33, y33 = filter_mag("-sys-Ext")
        x34, y34 = filter_mag("-survey")
        x35, y35 = filter_mag("-SNR")
...
ax1.scatter(d_768_sysurvey, d_644_sysurvey,  c= "red", alpha=0.8, marker='o', label='Symbiotics Survey')
ax1.scatter(d_768_cAlh, d_644_cAlh,  c= "greenyellow", alpha=0.8, marker='D', label='ALHAMBRA Candidates') 
ax1.scatter(d_768_ExtHII, d_644_ExtHII,  c= "gray", alpha=0.8, marker='D', label='H II in NGC 55 ')
...
plt.tight_layout()
plt.savefig('diagram-alhambra.pdf')
...
for line in f:
    line = line.strip()
    columns = line.split()
    mag1 = float(columns[21])
    mag2 = float(columns[25])
    mag3 = float(columns[37])
    d_mag1 = mag1 - mag2
    d_mag2 = mag1 - mag3
    d_644_cAlh.append(d_mag1)
    d_768_cAlh.append(d_mag2)
...
plt.xlabel('F582 - F830', size = 12)
plt.ylabel('F582 - F644', size = 12)
ax1.scatter(d_768, d_644, c='black', alpha=0.8, s=35, label='Halo PNe')
ax1.scatter(d_768_CNP, d_644_CNP,  c= "yellow", alpha=0.8, marker='o', label='Disk PN')
ax1.scatter(d_768_c, d_644_c, c='purple', alpha=0.8, label='CVs')
ax1.scatter(d_768_L2d, d_644_L2d,  c= "orange", alpha=0.8, marker='s', label='BB dddm1 L2')
...
ax1.scatter(d_768_sysurvey, d_644_sysurvey,  c= "red", alpha=0.8, marker='o', label='Symbiotics Survey')
ax1.scatter(d_768_cAlh, d_644_cAlh,  c= "greenyellow", alpha=0.8, marker='D', label='ALHAMBRA Candidates')
ax1.scatter(d_768_ExtHII, d_644_ExtHII, c= "gray" , alpha=0.8, marker='D', label='HII in NGC 55')
#ax1.scatter(d_768_SN, d_644_SN,  c= "black", alpha=0.8, marker='.', label='SN Remanents')
ax1.text(0.05, 0.95, 'Extinction, E0.1',
           transform=ax1.transAxes, fontsize='x-small')
for label_, x, y in zip(label, d_768, d_644):
...
ax1.minorticks_on()
#ax1.grid(which='minor')#, lw=0.3)
ax1.legend(scatterpoints=1, ncol=3, fontsize=6.0, loc="lower right", **lgd_kws)
#ax1.grid()
ax1.grid(which='minor', lw=0.5)
#sns.despine(bottom=True)
plt.tight_layout()
plt.savefig('diagram-alhambra-F582.pdf')                
