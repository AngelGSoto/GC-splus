#!/usr/bin/env python3
"""
Splus_photometry_gc_scientific_v17_FAST_SAFE_ERRORS_FIXED_RESUMABLE.py
VERSIÓN CIENTÍFICA PARA A&A - CON CAPACIDAD DE REANUDACIÓN
Script de fotometría científica para cúmulos globulares en S-PLUS
Optimizado para robustez y recuperación de fallos
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('splus_scientific_photometry_FAST_SAFE_RESUMABLE.log'),
...
class SPLUSPhotometryConfig:
    """Configuración OPTIMIZADA para máxima velocidad y robustez"""
    def __init__(self):
        ...
        # ⚡ MODO RÁPIDO: Desactivar gráficos para producción
...
# =============================================================================
# FUNCIONES DE REANUDACIÓN ROBUSTAS
# =============================================================================

def get_completed_fields():
    """Obtiene la lista de campos ya procesados de forma robusta"""
    completed_fields = []
    
    # Buscar archivos de salida individuales
    for i in range(1, 25):
        field_name = f'CenA{i:02d}'
        ...
        if os.path.exists(output_file):
            try:
                ...
                if file_size > 5000:  # Archivo razonablemente grande
                    df_test = pd.read_csv(output_file, nrows=5)
                    if len(df_test) > 0 and 'FLUX_F378_2' in df_test.columns:
                        completed_fields.append(field_name)
                        logging.info(f"✅ Campo {field_name} ya procesado ({file_size/1024/1024:.1f} MB)")
                    else:
                        logging.warning(f"⚠️  Campo {field_name} tiene estructura inválida, se reprocesará")
                else:
                    logging.warning(f"⚠️  Campo {field_name} tiene archivo muy pequeño ({file_size} bytes), se reprocesará")
            except Exception as e:
                logging.warning(f"⚠️  Error verificando {field_name}: {e}, se reprocesará")
    
    # Verificar también el archivo final combinado
    final_output = 'Results/all_fields_gc_photometry_FAST_SAFE_CORRECTED_ERRORS_v17.csv'
    if os.path.exists(final_output):
        try:
            final_size = os.path.getsize(final_output)
            logging.info(f"📁 Archivo final encontrado: {final_output} ({final_size/1024/1024:.1f} MB)")
        except Exception as e:
            logging.warning(f"Error verificando archivo final: {e}")
    
    logging.info(f"📊 Campos completados encontrados: {len(completed_fields)}")
    return completed_fields

def get_remaining_fields(completed_fields):
    """Obtiene la lista de campos pendientes con verificación"""
    all_fields = [f'CenA{i:02d}' for i in range(1, 25)]
    remaining_fields = [f for f in all_fields if f not in completed_fields]
    
    logging.info(f"📈 PROGRESO GLOBAL: {len(completed_fields)}/{len(all_fields)} campos completados")
    logging.info(f"🔄 CAMPOS PENDIENTES: {remaining_fields}")
    
    # Verificar que los campos pendientes existen en disco
    valid_remaining = []
    for field in remaining_fields:
        field_dir = field
        if os.path.exists(field_dir):
            # Verificar que tiene al menos algunos archivos FITS
            fits_files = [f for f in os.listdir(field_dir) if f.endswith('.fits') or f.endswith('.fits.fz')]
            if len(fits_files) >= len(config.filters):
                valid_remaining.append(field)
            else:
                logging.warning(f"❌ Campo {field} no tiene suficientes archivos FITS ({len(fits_files)} encontrados)")
        else:
            logging.warning(f"❌ Directorio {field} no existe")
    
    if len(valid_remaining) != len(remaining_fields):
        logging.warning(f"⚠️  Solo {len(valid_remaining)} de {len(remaining_fields)} campos pendientes son válidos")
    
    return valid_remaining

def safe_combine_results():
    """Combina resultados de forma segura sin duplicados - robusto"""
    try:
        all_dfs = []
        final_output = 'Results/all_fields_gc_photometry_FAST_SAFE_CORRECTED_ERRORS_v17.csv'
        
        # Si existe archivo final, usarlo como base
        if os.path.exists(final_output):
            try:
                existing_df = pd.read_csv(final_output)
                all_dfs.append(existing_df)
                logging.info(f"📥 Cargado archivo final existente: {len(existing_df)} fuentes")
            except Exception as e:
                logging.error(f"❌ Error cargando archivo final: {e}")
        
        # Agregar archivos individuales recientes
        for i in range(1, 25):
            field_name = f'CenA{i:02d}'
            individual_file = f'{field_name}_gc_photometry_FAST_SAFE_CORRECTED_ERRORS.csv'
            
            if os.path.exists(individual_file):
                try:
                    file_size = os.path.getsize(individual_file)
                    if file_size > 5000:  # Archivo razonable
                        field_df = pd.read_csv(individual_file)
                        if len(field_df) > 10:  # Tiene datos significativos
                            all_dfs.append(field_df)
                            logging.info(f"📥 Agregado {field_name}: {len(field_df)} fuentes")
                        else:
                            logging.warning(f"⚠️  {field_name} tiene muy pocos datos: {len(field_df)} fuentes")
                    else:
                        logging.warning(f"⚠️  {individual_file} es muy pequeño: {file_size} bytes")
                except Exception as e:
                    logging.error(f"❌ Error cargando {individual_file}: {e}")
        
        if all_dfs:
            # Combinar y eliminar duplicados robustamente
            combined_df = pd.concat(all_dfs, ignore_index=True)
            
            # Eliminar duplicados manteniendo el último
            original_count = len(combined_df)
            if 'recno' in combined_df.columns and 'FIELD' in combined_df.columns:
                combined_df = combined_df.drop_duplicates(subset=['recno', 'FIELD'], keep='last')
                duplicates_removed = original_count - len(combined_df)
                if duplicates_removed > 0:
                    logging.info(f"🧹 Eliminados {duplicates_removed} duplicados")
            
            # Guardar nuevo archivo combinado
            os.makedirs("Results", exist_ok=True)
            combined_df.to_csv(final_output, index=False)
            
            # Estadísticas finales
            fields_processed = combined_df['FIELD'].nunique() if 'FIELD' in combined_df.columns else 0
            logging.info(f"💾 COMBINACIÓN COMPLETADA: {len(combined_df)} fuentes únicas, {fields_processed} campos")
            
            return combined_df
        else:
            logging.warning("📭 No hay resultados válidos para combinar")
            return None
            
    except Exception as e:
        logging.error(f"❌ ERROR CRÍTICO en combinación: {e}")
        logging.error(traceback.format_exc())
        return None

# =============================================================================
# FUNCIONES CIENTÍFICAS PRINCIPALES (INALTERADAS)
# =============================================================================
...
def subtract_galaxy_background(data, median_box_size=25, gaussian_sigma=5):
    """RESTA DEL FONDO GALÁCTICO - método científico robusto"""
    try:
...
def detect_reference_stars_fast(data, error_map, header, nstars=15):
    """Detección de estrellas de referencia - optimizada"""
    try:
        ...
        logging.info(f"FAST DAOFind found {len(best_positions)} reference stars")
...
def analyze_growth_curves_fast(positions, data, error_map, header):
    """Análisis de curvas de crecimiento - método científico"""
    try:
        header_info = extract_header_information(header)
        pixel_scale = header_info['pixel_scale']
        seeing_fwhm = header_info['seeing_fwhm']
        ...
        for i, pos in enumerate(positions[:8]):  # Analizar solo las 8 más brillantes
            try:
                fluxes = []
                valid_radii = []
                ...
                for radius in growth_radii_pixels:
                    ...
                    if flux_abs > 0 and np.isfinite(flux_abs):
                        ...
                        valid_radii.append(radius * pixel_scale * 2)  # Convertir a diámetro en arcsec
                ...
                # Análisis de estabilidad en el plateau
                if len(valid_radii) > 3:
                    ...
                    if np.any(low_derivative_mask):
                        ...
                        plateau_radius = 6.0  # Aperture máxima por defecto
                else:
                    plateau_radius = min(valid_radii[-1], 6.0)
                
                # Limitar apertura máxima
                plateau_radius = min(plateau_radius, 6.0)
                
                # Solo aceptar si es razonablemente mayor que el seeing
...
def calculate_aperture_correction_fast(reference_positions, data, header):
    """Cálculo de corrección de apertura - método científico"""
    try:
        ...
        for pos in reference_positions[:10]:  # Usar solo 10 estrellas de referencia
            try:
                # Verificar bordes para apertura de 6"
                ...
                if (flux_2_abs > 0 and flux_3_abs > 0 and flux_6_abs > 0 and
                    ...
                    # Validar correcciones físicas
                    if (0 < corr_2 < 1.0 and 0 < corr_3 < 1.0 and 
                        corr_3 < corr_2):  # Corrección debe ser menor para apertura mayor
...
def load_weight_map_splus_corrected_fast(weight_path, data_shape, header, data_original):
    """
    Carga de weight maps de S-PLUS - robusta
    """
    try:
        ...
        if ratio > 10.0:  # Ratio poco realista
            logging.warning(f"⚠️  Weight map ratio suspicious: {ratio:.3f}")
...
def calculate_background_error_fast(annulus_data_1d, annulus_error_1d, aperture_area):
    """
    Cálculo robusto del error del fondo usando MAD
    """
    ...
    # Error usando MAD (Median Absolute Deviation) - robusto a outliers
    bkg_mad = mad_std(annulus_data_1d)
    bkg_error_per_pixel = 1.253 * bkg_mad / np.sqrt(len(annulus_data_1d))  # Factor 1.253 para MAD -> σ
    total_bkg_error = bkg_error_per_pixel * aperture_area
    
    return bkg_median, total_bkg_error

def process_single_filter_fast(args):
    """Procesamiento para un solo filtro - con errores corregidos"""
    try:
        ...
        if not image_path:
            logging.warning(f"❌ {filter_name}: Image file not found")
        ...
        with fits.open(image_path) as hdul:
            for hdu in hdul:
                ...
                logging.warning(f"❌ {filter_name}: No valid HDU found")
                return None, filter_name
        ...
        if error_map is None:
            ...
            logging.info(f"⚡ {filter_name}: Using fallback error map")
        
        # Validar posiciones
        if len(valid_positions) == 0:
            logging.warning(f"❌ {filter_name}: No valid positions")
            return None, filter_name
        ...
        if np.sum(valid_mask) == 0:
            logging.warning(f"❌ {filter_name}: No positions within safe margins")
            return None, filter_name
        ...
        # Usar imagen RESIDUAL para fotometría
        data_for_detection = data_residual
        data_for_photometry = data_residual
        
        # Detección de estrellas de referencia
        ...
        if len(reference_stars) < config.min_reference_stars:
            ...
            logging.info(f"⚡ {filter_name}: Using {len(reference_stars)} catalog positions as reference")
        ...
        for aperture_diam in config.aperture_diams:
            ...
            # Filtrar posiciones válidas para fotometría
            ...
            # Cálculo MEJORADO del fondo y errores
            ...
            # Límites conservadores para errores
        ...
        logging.info(f"✅ {filter_name}: FAST processing completed - {valid_measurements} valid measurements")
...
class SPLUSGCScientificPhotometryFAST:
    """Pipeline científico para fotometría de cúmulos globulares en S-PLUS"""
    ...
    def __init__(self, catalog_path, zeropoints_file, debug=False, n_workers=None):
        ...
        # Workers optimizados
        self.n_workers = min(n_workers or 8, 12)
        
        logging.info("🚀 INITIALIZED SCIENTIFIC S-PLUS PHOTOMETRY PIPELINE")
        logging.info(f"   ⚡ PARALLEL WORKERS: {self.n_workers}")
        logging.info("   🛡️  CORRECTED ERROR ESTIMATION")
        logging.info("   🔬 SCIENTIFIC GRADE FOR A&A PUBLICATION")
    
    def find_splus_file(self, field_name, filter_name):
        """Encuentra archivos S-PLUS de forma robusta"""
    ...
    def is_source_in_field(self, ra, dec, field_ra, field_dec, radius=0.84):
        """Verifica si una fuente está dentro del campo usando astropy"""
    ...
    def process_field_fast(self, field_name):
        """Procesamiento científico robusto del campo"""
        logging.info(f"🔬 Processing field {field_name} - SCIENTIFIC GRADE")
        start_time = time.time()
        
        if not os.path.exists(field_name):
            logging.error(f"❌ Directory {field_name} does not exist")
            return None
            
        if field_name not in self.zeropoints:
            logging.warning(f"⚠️  No zeropoints for field {field_name}")
            return None
            
        first_filter_img = self.find_splus_file(field_name, self.filters[0])
        if not first_filter_img:
            logging.error(f"❌ No image files found for field {field_name}")
            return None
        ...
        try:
            with fits.open(first_filter_img) as hdul:
                for hdu in hdul:
                    ...
                    logging.error(f"❌ No valid HDU in {first_filter_img}")
                    return None
        except Exception as e:
            logging.error(f"❌ Error opening {first_filter_img}: {e}")
            return None
        
        field_ra, field_dec = header.get('CRVAL1'), header.get('CRVAL2')
        if field_ra is None or field_dec is None:
            logging.warning(f"⚠️  No CRVAL in header for {field_name}")
            return None
        
        # Preprocesamiento del catálogo
        ...
        if len(field_sources) == 0:
            logging.warning(f"⚠️  No sources in field {field_name}")
            return None
        ...
        try:
            ...
            logging.error(f"❌ WCS conversion failed for {field_name}: {e}")
            return None
        ...
        if len(valid_positions) == 0:
            logging.warning(f"⚠️  No valid positions in field {field_name} after margin check")
            return None
            
        results_df = field_sources.copy()
        successful_filters = 0
        
        # Procesar filtros en serie para mayor estabilidad
        ...
        if successful_filters > 0:
            ...
            results_df['PHOTOMETRY_METHOD'] = 'S-PLUS_SCIENTIFIC_v17_CORRECTED_ERRORS'
            results_df['PRIMARY_APERTURE'] = '2 arcsec'
            results_df['GALAXY_SUBTRACTION'] = 'APPLIED'
            results_df['ERROR_METHOD'] = 'WEIGHT_MAP_CORRECTED_ROBUST'
            results_df['ZEROPOINT_SOURCE'] = 'EMPIRICAL_CALIBRATION'
            ...
            logging.error(f"❌ No successful filters for {field_name}")
            return None

def process_single_field_wrapper(args):
    """Wrapper robusto para procesar un campo individual"""
    photometry, field_name = args
    try:
        logging.info(f"🚀 Starting processing for {field_name}")
        result = photometry.process_field_fast(field_name)
        if result is not None:
            logging.info(f"✅ Successfully processed {field_name}")
        else:
            logging.error(f"❌ Failed to process {field_name}")
        return result
    except Exception as e:
        logging.error(f"❌ CRITICAL ERROR processing {field_name}: {e}")
        logging.error(traceback.format_exc())
        return None

def main_scientific_resumable():
    """Función principal CIENTÍFICA con capacidad de reanudación"""
    logging.info("=" * 80)
    logging.info("🔬 S-PLUS GLOBULAR CLUSTER PHOTOMETRY - SCIENTIFIC GRADE")
    logging.info("   🏆 QUALITY FOR A&A PUBLICATION")
    logging.info("   🛡️  ROBUST RESUMPTION CAPABILITY")
    logging.info("   ⚡ OPTIMIZED PARALLEL PROCESSING")
    logging.info("=" * 80)
    
    # Verificar campos completados
    completed_fields = get_completed_fields()
    remaining_fields = get_remaining_fields(completed_fields)
    
    if not remaining_fields:
        logging.info("🎉 ¡ALL FIELDS ALREADY PROCESSED!")
        logging.info("🔍 Performing final combination...")
        final_df = safe_combine_results()
    ...
    n_workers = min(6, len(remaining_fields))  # Conservador para estabilidad
    
    logging.info(f"🛡️  Processing {len(remaining_fields)} fields with {n_workers} scientific workers")
    
    # Inicializar pipeline científico
    ...
    # Procesar campos pendientes con agrupación robusta
    completed_count = 0
    total_to_process = len(remaining_fields)
    
    # Grupos pequeños para mayor estabilidad
    group_size = min(3, n_workers)
    
    for i in range(0, total_to_process, group_size):
        group_fields = remaining_fields[i:i + group_size]
        group_number = (i // group_size) + 1
        total_groups = (total_to_process + group_size - 1) // group_size
        
        logging.info(f"📦 Processing group {group_number}/{total_groups}: {group_fields}")
        ...
        with Pool(processes=min(len(group_fields), n_workers)) as pool:
            ...
            for result in tqdm(pool.imap(process_single_field_wrapper, args_list), 
                ...
                if result is not None:
                    ...
                    # Combinación parcial cada campo para máxima seguridad
                    logging.info(f"🔄 Safe combination after {completed_count}/{total_to_process} fields")
                    safe_combine_results()
                    
                    progress_percent = (completed_count / total_to_process) * 100
                    logging.info(f"📊 OVERALL PROGRESS: {completed_count}/{total_to_process} "
                               f"({progress_percent:.1f}%)")
    
    # Combinación final y verificación
    logging.info("🎉 ALL FIELDS PROCESSED. Performing final scientific combination...")
    final_df = safe_combine_results()
    
    if final_df is not None:
        # Verificación científica final
        total_sources = len(final_df)
        fields_processed = final_df['FIELD'].nunique() if 'FIELD' in final_df.columns else 0
        
        logging.info("🏁 SCIENTIFIC PROCESSING COMPLETED SUCCESSFULLY")
        logging.info(f"📊 FINAL STATISTICS:")
        logging.info(f"   • Total sources: {total_sources}")
        logging.info(f"   • Fields processed: {fields_processed}/24")
        logging.info(f"   • Output file: Results/all_fields_gc_photometry_FAST_SAFE_CORRECTED_ERRORS_v17.csv")
        
        # Verificar completitud
        if fields_processed == 24:
            logging.info("✅ ALL 24 FIELDS SUCCESSFULLY PROCESSED")
        else:
            missing = 24 - fields_processed
            logging.warning(f"⚠️  {missing} FIELDS MISSING FROM FINAL OUTPUT")
    else:
        logging.error("❌ FINAL COMBINATION FAILED")

if __name__ == "__main__":
    # Configuración para máxima estabilidad científica
    os.environ['OMP_NUM_THREADS'] = '1'
    os.environ['MKL_NUM_THREADS'] = '1'
    os.environ['OPENBLAS_NUM_THREADS'] = '1'
    os.environ['NUMEXPR_NUM_THREADS'] = '1'
    
    # Iniciar procesamiento científico
    main_scientific_resumable()
