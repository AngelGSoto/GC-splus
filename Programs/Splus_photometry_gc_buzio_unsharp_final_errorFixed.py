#!/usr/bin/env python3
"""
Splus_photometry_gc_scientific_v17_corrected_errors.py
VERSIÓN CORREGIDA: Corrección específica para estimación de errores
- Mantiene toda la funcionalidad original
- Corrige la sobreestimación de errores usando weight maps de S-PLUS
- Basado en headers reales de S-PLUS
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('splus_scientific_photometry_v17_corrected_errors.log'),
...
# =============================================================================
# CORRECCIONES ESPECÍFICAS PARA ESTIMACIÓN DE ERRORES
# =============================================================================
...
class SPLUSGCScientificPhotometryOptimized:
    """Pipeline OPTIMIZADO con corrección de errores"""
    ...
    def __init__(self, catalog_path, zeropoints_file, debug=False):
        ...
        logging.info("🎯 INITIALIZED S-PLUS PHOTOMETRY PIPELINE v17 WITH CORRECTED ERRORS")
        logging.info("   - ERROR ESTIMATION: Corrected using weight maps and robust statistics")
        logging.info("   - BACKGROUND ERROR: Using MAD and proper error propagation")
        logging.info("   - ERROR VALIDATION: Automatic quality checks")
    ...
    def process_field_optimized(self, field_name):
        """Procesamiento OPTIMIZADO con errores corregidos"""
        ...
        for filt in tqdm(self.filters, desc=f"Processing {field_name}"):
            ...
            try:
                ...
                if result is not None:
                    ...
                    logging.info(f"✅ {filter_name}: CORRECTED ERROR results integrated")
            except Exception as e:
...
def main():
    """Función principal con errores corregidos"""
    logging.info("=" * 80)
    logging.info("🎯 S-PLUS GLOBULAR CLUSTER PHOTOMETRY v17 - CORRECTED ERRORS")
    logging.info("   CORRECCIONES: Propagación adecuada de errores desde weight maps")
    logging.info("   ERRORES: Cálculo correcto del error del fondo usando weight maps")
    logging.info("   MÉTODO: Error de mediana = 1.253 * MAD / √N")
    logging.info("   VALIDACIÓN: Chequeo automático de calidad de errores")
    logging.info("=" * 80)
    ...
    for field in tqdm(fields, desc="Processing S-PLUS fields"):
        ...
        if results is not None and len(results) > 0:
            ...
            output_file = f'{field}_gc_photometry_corrected_errors_v17.csv'
            results.to_csv(output_file, index=False)
            logging.info(f"✅ Saved {field} CORRECTED ERROR results to {output_file}")
    
    if all_results:
        final_results = pd.concat(all_results, ignore_index=True)
        os.makedirs("Results", exist_ok=True)
        output_file = 'Results/all_fields_gc_photometry_corrected_errors_v17.csv'
        final_results.to_csv(output_file, index=False)
        
        logging.info("🎉 S-PLUS PHOTOMETRY WITH CORRECTED ERRORS COMPLETED SUCCESSFULLY")
        logging.info("   ✅ ERROR PROPAGATION: Correct weight map usage")
        logging.info("   ✅ BACKGROUND ERROR: Proper median error calculation using MAD")
        logging.info("   ✅ TOTAL ERROR: √(σ_raw² + σ_bkg²) with reasonable limits")
        logging.info("   ✅ VALIDATION: Automatic error quality checks")
