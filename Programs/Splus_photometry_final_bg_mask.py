        ...
        for filter_name in self.filters:
            ...
            try:
                ...
                for aperture_size in self.apertures:
                    ...
                    mag = [zero_point - 2.5 * np.log10(f) if f > 0 else 99.0 for f in flux]
                    mag_err = [(2.5 / np.log(10)) * (fe / f) if f > 0 else 99.0 for f, fe in zip(flux, flux_err)]
                    snr = [f / fe if fe > 0 else 0.0 for f, fe in zip(flux, flux_err)]
                    
                    # Store in DataFrame
                    results[f'X_{filter_name}_{aperture_size}'] = positions[:, 0]
                    results[f'Y_{filter_name}_{aperture_size}'] = positions[:, 1]
                    results[f'FLUX_{filter_name}_{aperture_size}'] = flux
                    results[f'FLUXERR_{filter_name}_{aperture_size}'] = flux_err
