#!/usr/bin/env python3
# preparar_cigale_redshift_primero_decam.py
...
def preparar_datos_redshift_primero(input_file, output_file="gc_splus_cigale_custom.txt", n_objects=None):
    ...
    decam_filter_map = {
        'u': 'sdss.up',  # Taylor column: umag
        'g': 'sdss.gp',  # Taylor column: gmag
        'r': 'sdss.rp',  # Taylor column: rmag
        'i': 'sdss.ip',  # Taylor column: imag
        'z': 'sdss.zp'   # Taylor column: zmag
    }
    ...
    for idx, row in df.iterrows():
        ...
        # 1. PROCESAR FILTROS S-PLUS (7 filtros)
        for internal_name, cigale_name in splus_filter_map.items():
            mag_col = f'MAG_{internal_name}_3'      # Magnitud (apertura 3 arcsec)
            err_col = f'MAGERR_{internal_name}_3'   # Error
        ...
        # 2. PROCESAR FILTROS DECam (5 filtros) - Catálogo Taylor
        decam_bands = ['u', 'g', 'r', 'i', 'z']
        
        for band in decam_bands:
            mag_col = f'{band}mag'      # Ejemplo: umag, gmag, etc.
            err_col = f'e_{band}mag'    # Ejemplo: e_umag, e_gmag, etc.
            
            # Verificar si las columnas existen
            if mag_col not in df.columns:
                print(f"⚠️  ADVERTENCIA: Columna {mag_col} no encontrada en archivo")
                mag = 99.0
                err = 0.1
            else:
                mag = row.get(mag_col, 99.0)
...
if __name__ == "__main__":
    ...
    N_OBJECTS_TEST = 200  # None para todos, o un número para prueba
    
    print("=" * 78)
    print("📁 PROCESAMIENTO DE DATOS PARA CIGALE")
    print("=" * 78)
    ...
    if success:
        print("\n" + "=" * 78)
        print("🚀 CONFIGURACIÓN PARA CIGALE:")
        print("=" * 78)
