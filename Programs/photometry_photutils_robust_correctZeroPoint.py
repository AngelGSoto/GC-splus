...
class SPLUSPhotometry:
    def __init__(self, catalog_path, zeropoints_file):
        ...
        # Cargar zeropoints desde el archivo
        ...
        # Crear diccionario de zeropoints
        self.zeropoints = {}
        for _, row in self.zeropoints_df.iterrows():
            self.zeropoints[row['filter']] = row['zeropoint'] + 20.0  # Sumar 20 para obtener el zeropoint final
        ...
        # Mostrar los zeropoints que se están usando
        print("\nZeropoints utilizados (desde archivo):")
        for filter_name in self.filters:
            if filter_name in self.zeropoints:
                print(f"  {filter_name}: {self.zeropoints[filter_name]:.3f}")
            else:
                print(f"  {filter_name}: No encontrado en el archivo de zeropoints")
        ...
        for filter_name in tqdm(self.filters, desc="Processing filters"):
            try:
                ...
                if not os.path.exists(sci_file):
                    print(f"Advertencia: Archivo científico {sci_file} no encontrado")
                    continue
                
                if not os.path.exists(weight_file):
                    print(f"Advertencia: Archivo de peso {weight_file} no encontrado")
                    # Crear un mapa de peso uniforme si no existe
                    with fits.open(sci_file) as hdul:
                        if len(hdul) > 1:
                            data_shape = hdul[1].data.shape
                        else:
                            data_shape = hdul[0].data.shape
                    weight_data = np.ones(data_shape)
                    error_data = np.ones(data_shape)  # Error uniforme
                    print(f"Usando mapa de peso uniforme para {filter_name}")
                else:
                    # Cargar mapa de pesos real
    ...
    def measure_source(self, source, data, error_data, wcs, filter_name):
        ...
        try:
            ...
            if filter_name not in self.zeropoints:
                print(f"Advertencia: No hay zeropoint para {filter_name}")
                return None
            
            zp = self.zeropoints[filter_name]
            ...
            # Control de calidad - umbral más bajo para incluir más fuentes
            if snr < 2:  # Reducido de 3 a 2 para incluir fuentes más débiles
                return None
...
if __name__ == "__main__":
    ...
    zeropoints_file = 'cenA01_zeropoints_aa.csv'
    ...
    if not os.path.exists(catalog_path):
        ...
        try:
            ...
            if len(results) > 0:
                ...
                # Guardar resultados
                output_file = 'ngc5128_splus_photometry_calibrated.csv'
                ...
                # Análisis específico para nuestros filtros calibrados
                print("\n=== FILTROS CALIBRADOS ===")
                for filter_name in photometry.filters:
                    mag_col = f'MAG_{filter_name}'
                    valid = results[mag_col].notna()
                    if valid.any():
                        print(f"{filter_name}:")
                        print(f"  Rango de magnitudes: {results[mag_col][valid].min():.2f} - {results[mag_col][valid].max():.2f}")
                        print(f"  Mediana: {results[mag_col][valid].median():.2f}")
            else:
